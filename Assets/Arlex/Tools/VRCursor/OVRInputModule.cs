...
#region PointerEventData pool
        ...
        {
			var pressed = Input.GetKeyDown(gazeClickKey) || OVRInput.GetDown(joyPadClickButton)||gazed;
			var released = Input.GetKeyUp(gazeClickKey) || OVRInput.GetUp(joyPadClickButton)||gazed;
		...
		protected void PointerChoosing(RaycastResult raycast){
			...
			//change to another raycast
			if (raycast.gameObject.name != lastraycaset) {
				GameObject.Find ("Loading").GetComponent<PointerLoading> ().resetloading ();
				isgazing = false;
			}
		}
    };
